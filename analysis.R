...
# Note: Install missing packages before running if necessary
...
train_index <- createDataPartition(train_data$Irrigation_Need, p = 0.8, list = FALSE)
...
rf_model <- randomForest(Irrigation_Need ~ ., data = train_set, ntree = 100, importance = TRUE)
...
conf_matrix <- confusionMatrix(factor(valid_predictions), factor(valid_set$Irrigation_Need))
